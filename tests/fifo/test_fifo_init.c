...
int main(void)
{
    ...
    data_length = 0U;
    expected_result = PRA_BOOL_FALSE;
    expected_ec = PRA_FIFO_EC_INVALID_LENGTH;
    actual_ec = PRA_FIFO_EC_NONE;
    if (expected_result != pra_fifo_init(
                               &fifo,
                               data,
    ...
    if (expected_result != pra_fifo_init(
                               ...
                               &actual_ec) ||
        expected_ec != actual_ec ||
        expected_data_length != fifo.data_length ||
        expected_used_length != fifo.used_length ||
        expected_next_r_pos != fifo.next_r_pos ||
        expected_next_W_pos != fifo.next_w_pos)
    {
        result |= err_error4;
    }
    else
    {
        ...
        {
            if (expected_data_value != fifo.p_data[i])
            {
                result |= err_error5;
                break;
            }
